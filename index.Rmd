...
* <span style = 'font-size:80%;'>**`www` directory:** the directory expected by a Shiny application to locally store the elements that will be rendered in the web browser (and are not the output of the scripts); includes image files (e.g. `.png`, `.jpeg`), HTML, CSS, and JavaScript files, etc.</span>
]
...
<span style = 'font-size:80%;'>Use the appropriate `render*()` function to make your output reactive (e.g. if you have a `plotOutput` in your UI, you will need to use `renderPlot()` in your server. Within your `render*()`,  write any code *inside* a set of curly braces, `{}`. This allows you to include as many lines of code as it takes to build your object.</span>
```{r, eval = FALSE}
# user interface ----
ui <- fluidPage(
