...
---
### **Coding Exercise:** Solution for adding a `DT` table
...
---
### **Coding Exercise:** Outline your app's layout
...
* <span style = 'font-size:80%;'>It might be easiest to comment out your current application code and write a "fresh" empty application, which we will later fill in with our text, inputs, outputs, etc.</span>

---
### **Example 1:** a multi-row layout using `fluidRows()`
...
---
### **Example 2:** a navigation bar with `tabPanel()`s and `tabsetPanel()`s
...
---
### **Coding Exercise:** Add inputs & outputs back into your app

<span style = 'font-size:80%;'>Now that you have the bones of your application's layout coded up, it's time to add back in your inputs and outputs.</span>

**Some tips to get you started:**
* <span style = 'font-size:80%;'>If you have not yet done so, add comments or text to your layout functions specifying which elements should go where (e.g. `# scatterplot sliderInput goes here`) </span>
* <span style = 'font-size:80%;'>Copy inputs and outputs from our original app (the one build just using `fluidPage()` to contain all of our elements) and paste them into the new application's layout function framework</span>
...
* <span style = 'font-size:80%;'>Remember to separate elements with a `,`</span>

<span style = 'font-size:80%;'>If you want to reproduce the app I'm building, start with the layout structure detailed on [slide #54]() (*Example 2: a navigation bar with `tabPanel()`s and `tabsetPanel()`s*) </span>

---
### My app (if you're following along with this example):
