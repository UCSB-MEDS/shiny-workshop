...
.pull-right[
* <span style = 'font-size:65%;'>**app directory (e.g. the `myapp` folder):** stores your app (`app.R`) and all other scripts/files/etc. used by your app</span>
* <span style = 'font-size:65%;'>**`global.R`:** a script for storing commonly used pieces of logic; not required, but can reduce redundant code and increase the app's speed; must be in the same directory as `app.R`</span>
* <span style = 'font-size:65%;'>**data directory (e.g. the `data` folder):** stores the data used by your app; alternatively, you can [store your data remotely](https://shiny.rstudio.com/articles/persistent-data-storage.html) (e.g. Google Sheets, Dropbox)</span>
* <span style = 'font-size:65%;'>**`www` directory:** a directory expected by Shiny that stores any elements to be rendered in the web browser (and are not the output of the scripts); includes image files (e.g. `.png`, `.jpeg`), HTML, CSS, and JavaScript files, etc.</span>
* <span style = 'font-size:65%;'>**`R` directory (outside of app folder):** a place for any scripts not used by your app; I find that building my outputs first in a separate script helps me from going cross-eyed -- I store them here </span>
]
...
* <span style = 'font-size: 80%;'>Though it's *not* a shiny framework, [`{flexdashboard}`](https://pkgs.rstudio.com/flexdashboard/index.html) allows you to produce interactive dashboards using R Markdown. Find example projects and their source code [here](https://pkgs.rstudio.com/flexdashboard/articles/examples.html).</span>
