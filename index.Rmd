...
Before we get started, make sure to set up a GitHub repository and R project to house our soon-to-be Shiny app. I'll call mine something like, "practice-shiny".
...
* <span style = 'font-size:70%;'>adding a second `sliderInput` for our reactive histogram output (**Exercise 5**)</span>  

--

<span style = 'font-size:70%;'>Make it pretty:</span>

* <span style = 'font-size:70%;'>apply a theme using the `{bslib}` package (**Exercise 6**)</span>
* <span style = 'font-size:70%;'>add images to  your app(**Exercise 7**)</span>
...
* <span style = 'font-size:70%;color:#c9c6c5;'>adding a second `sliderInput` for our reactive histogram output (**Exercise 5**)</span>  

<span style = 'font-size:70%;color:#c9c6c5;'>Make it pretty:</span>

* <span style = 'font-size:70%;color:#c9c6c5;'>apply a theme using the `{bslib}` package (**Exercise 6**)</span>
* <span style = 'font-size:70%;color:#c9c6c5;'>add images to  your app(**Exercise 7**)</span>
...
.pull-right[
* <span style = 'font-size:80%;'>**app directory (e.g. the `myapp` folder):** stores your app (`app.R`) and all other scripts/files/etc. used by your app</span>
* <span style = 'font-size:80%;'>**`global.R`:** a script for storing commonly used pieces of logic; not required for a Shiny app, but can reduce redundant code and increase the app's speed; must be in the same directory as `app.R`</span>
* <span style = 'font-size:80%;'>**data directory (e.g. the `data` folder):** stores the data used by your app; alternatively, you can store your data externally</span>
* <span style = 'font-size:80%;'>**`www` directory:** a directory expected by Shiny that stores any elements to be rendered in the web browser (and are not the output of the scripts); includes image files (e.g. `.png`, `.jpeg`), HTML, CSS, and JavaScript files, etc.</span>
]
...
* <span style = 'font-size:80%;'>Sketch out your layout with a pen and paper first! It's worth noting that you'll likely choose to start with this step (followed by coding up your UI layout) before buidling any inputs/outputs/etc. as you start to build your own apps.</span>
...
<span style = 'font-size:80%;'>3. Give it a minute (or a few) to deploy. It should open up a browser window with your republished app once processed.</span>  
...
* <span style = 'font-size:70%;color:#000000;'>adding a second `sliderInput` for our reactive histogram output (**Exercise 5**)</span>
...
<span style = 'font-size:70%;color:#000000;'>Make it pretty:</span>

* <span style = 'font-size:70%;color:#000000;'>apply a theme using the `{bslib}` package (**Exercise 6**)</span>
* <span style = 'font-size:70%;color:#000000;'>add images to  your app(**Exercise 7**)</span>
...
---
### **Exercise 6:** Add a theme to your app using `{bslib}`
...
---
### <span style = 'font-size:90%;'>**Exercise 7:** Add an image(s) to your app</span>
...
.footnote[
Reactivity is *confusing*. I recommend watching Barret Schloerke's talk, linked above, and reading through the [Shiny Reactlog vignette ](https://rstudio.github.io/reactlog/articles/reactlog.html).
]
...
<span style = 'font-size: 80%;'>**3.** Use the keyboard shortcut `cmd`/`cntrl` + `F3` to launch reactlog</span>  
...
<span style = 'font-size: 80%;'>I've created two apps, one that's working the way I expect and another that's not. You can fork and clone both apps below if you'd like to run the code yourself.</span> 
...
<span style = 'font-size: 80%;'>Let's try out reactlog on an intentionally borked app. Here, our images change when the `radioButton` user inputs are updated, but our text doesn't appear.</span>

<span style = 'font-size: 80%;'>As in our functioning app, the **All penguins** image is selected by default. For demonstration purposes, I'll select each option moving down the list (**Sassy chinstrap**, **Staring gentoo**, **Adorable adelie**) before launching reactlog.</span>
...
<span style = 'font-size: 70%;'>As we click down the list of `radioButton`s, the image output is invalidated, then updated accordingly but the text output remains disconnected from our input.</span>
...
<span style = 'font-size: 80%;'>This suggests that it might be wise to ensure that our UI and server are actually able to communicate about our desired text output. After careful inspection of our `textOutput()` and `renderText()` code, we find that a spelling error is to blame:</span>

<span style = 'font-size: 80%;'>Our `outputId` in the **UI** is set to `penguin_text`:</span>
```{r, echo = TRUE, eval = FALSE}
ui <- fluidPage(
...
<span style = 'font-size: 80%;'>But we call `penguins_text` when rendering our output in the **server**:</span>
```{r, echo = TRUE, eval = FALSE}
server <- function(input, output){
...
* `r fontawesome::fa("tachometer-alt", fill = "#93C6C2", a11y = "sem")` {shinydashboards} (https://rstudio.github.io/shinydashboard/)
* `r fontawesome::fa("star", fill = "#93C6C2", a11y = "sem")` golem (https://github.com/ThinkR-open/golem)
