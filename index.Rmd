...
<span style = 'font-size: 85%;'>`r fontawesome::fa("angle-right", fill = "#05859B", a11y = "sem")` We will be using a handful of R packages throughout this workshop. You can install/update them all at once by running:</span>  
...
.center[
```{r, out.width = '25%', out.height = '25%', echo = FALSE, fig.alt = ""}
...
Here, we'll add our first reactive components, explore different layouts, and establish a workflow. COME BACK TO THIS
...
ui <- fluidPage(
  ...
  # app subtitle ----
  "Exploring Palmer Penguins and Antarctic Temperatures"
...
ui <- fluidPage(
  ...
  # app subtitle ----
  p(strong("Exploring Palmer Penguins and Antarctic Temperatures"))
...
Some examples: 
```{r, eval = FALSE}
plotOutput(outputId = "bodySize_plot")

dataTableOutput(outputId = "bodySize_dt")
```
...
.footnote[
Read more about reactivity in [Ch.3 Basic Reactivity](https://mastering-shiny.org/basic-reactivity.html#basic-reactivity) of [Mastering Shiny](https://mastering-shiny.org/index.html).
]
...
<br>
--
**2.** Build objects to display using a `render*()` function  

<br>
--
**3.** Use input values (from the UI) with `input$<id>`  

---
### **Rule 1:** Save objects you want to display to `output$`

<span style = 'font-size:80%;'>In our **UI**, we created a placeholder for our plot output using the `plotOutput()` function and gave it an Id called `"bodySize_scatterplot"`. We will now tell our server how to generate that plot output AHHHHHH</span>
```{r, eval = FALSE}
# user interface ----
ui <- fluidPage(
...
server <- function(input, output) {
  ...
  output$bodySize_scatterPlot <- # code here #<<
    
}
```

.footnote[
For another example, check out Shiny's Tutorial, [Display Reactive Output](https://shiny.rstudio.com/tutorial/written-tutorial/lesson4/).
]
---
...
---
### **Rule 3:** Use input values (from the UI) with `input$`
